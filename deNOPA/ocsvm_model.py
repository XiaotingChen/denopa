...
def __reads_in_peaks(sam_list, peak_chr, peak_start, peak_stop, frag_len, left_shift=+4, right_shift=-5,
                     return_ends=True):
    """
...
def __calc_ov_frags(para):
    ...
    reads, read_ends = __reads_in_peaks(sam_list, peak_chr, peak_start, peak_stop, frag_len, left_shift=left_shift,
                                        right_shift=right_shift)

    # Initialize to determine if the peak is dynamic.
    isDynamic = determine_dynamic.DetermineDynamics(reads, smooth_file, (peak_chr, peak_start, peak_stop), dp)
    ...
    def coverage_pvalue(record):
        ...
        p = sum(
            [pl * double(l - L + 1) / double(l) for l, pl in peak_length_dis.items() if l >= L]
        )
    ...
    def end_pvalue(record):
        ...
        return [lmd, poisson(mean_end_per_base * (record[4] - record[2] + 1)).cdf(record[idx] - 1)]

    cand_cp[cand_cp.shape[1]] = cand_cp.apply(end_pvalue, axis=1)
    cand_cp[cand_cp.shape[1]] = cand_cp.apply(lambda u: 1 - (1 - u[9][2]) * (1 - u[11][1]), axis=1)
...
def calc_ov_frags(sam_list, cand_mm, peaks, frag_len, left_shift, right_shift, smooth_file, dynamic_pvalue=0.05,
                  proc=1):
    pool = mp.Pool(proc)
    cand_split = {}
    ...
    params = []
    for k, v in cand_split.iteritems():
        params.append(
            (
                pd.DataFrame(v),
                peaks.loc[k, :],
...
class FinalModeling(object):
    ...
    def getFeatures(self):
        ...
        neg = set(self.neg)
        naive = self.raw_data.loc[[i for i in self.raw_data.index if not i in neg], [0, 1, 5]].copy()
    ...
    def featureTransform(self):
        transFun = fun = lambda x: (x <= 0) * ((2 * norm(0, 1).pdf(0)) * x) + (x > 0) * 2 * (norm(0, 1).cdf(x) - 0.5)
        self.transformed_features = pd.DataFrame(index=self.raw_features.index, columns=self.raw_features.columns)
        self.transformed_features[0] = self.raw_features[0] / std(self.raw_features[0])
        self.transformed_features[1] = transFun(
            (self.raw_features[1] - mean(self.raw_features[1])) / std(self.raw_features[1])
        )
        self.transformed_features[1] = self.transformed_features[1] / std(self.transformed_features[1])
        self.transformed_features[2] = transFun(
            -(self.raw_features[2] - mean(self.raw_features[2])) / std(self.raw_features[2])
        )
        self.transformed_features[2] = self.transformed_features[2] / std(self.transformed_features[2])
