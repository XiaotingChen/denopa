...
(x > 0) * 2 * (norm(0, 1).cdf(x) - 0.5)
        ...
        def AT(a, b):
            m = b[0]
            return -(2 * sqrt(a + 0.375) - (2 * sqrt(m + 0.375) - 1 /
                                            (4 * sqrt(m))))
        ...
        y = asarray(
            [BAT(a, b, c[1]) for a, b, c in zip(data[8], data[7], data[9])])
        y = y - mean(y)
    ...
    def choosePara(self):
        ...
        self.eps = percentile(dis[:, 1], self.qeps * 100.)
        nbs = kdtree.query_radius(self.transData, self.eps,
                                  count_only=True) - 1
    ...
    def detectSig(self, rawData):
        rawData[12] = [
            1 - (1 - p[1]) * (1 - q[2])
            for p, q in zip(rawData[11], rawData[9])
        ]
        self.sig = multipletests(rawData[12],
                                 method="fdr_bh",
                                 alpha=self.adjusted_p)[0]
    ...
    def __call__(self, rawData, label=None):
        ...
        if label == None:
            ...
            label = self.label | (zeros_like(self.label, dtype=bool)
                                  if self.dbscan_only else self.sig)
        else:
            label = label
        x = []
        for _, v in rawData.loc[label, :].copy().iterrows():
            x.append([
                ...
                int(v[2]),
                int(v[3]),
                int(v[4]) + 1, v[7], v[8], v[10], v[9][2], v[11][1], v[12],
                v[13]
            ])
        x = pd.DataFrame(x)
...
class CandidatesWithNOC(object):
    ...
    def addToCand(self):
        def aPvalue(x, n):
            an = (2 * log(n))**(-0.5)
            bn = (2 * log(n))**0.5 - 0.5 * (
                (2 * log(n))**(-0.5)) * (log(log(n)) + log(4 * pi))
            t = (x - bn) / an
            return exp(-exp(-t))
        ...
        with h5py.File(self.nocFile, 'r') as hdf:
            for ix, (c, a, b) in enumerate(
                    zip(self.cand[0], self.cand[2].astype(int),
                        self.cand[4].astype(int) + 1)):
...
class varCandidatesWithNOC(object):
    ...
    def addToCand(self):
        ...
        y = []
        flag = []
        with h5py.File(self.nocFile, "r") as hdf, h5py.File(self.nosFile,
                                                            "r") as sdf:
            for ix, (c, a, b) in enumerate(
                    zip(self.cand[0], self.cand[2].astype(int),
                        self.cand[4].astype(int) + 1)):
                ...
                if len(f) > 0:
                    ...
                    y.append([
                        p, q,
                        aPvalue((q - mean(s0)) / std(s0), len(s0)),
                        len(s0)
                    ])
                    flag.append(True)
                else:
